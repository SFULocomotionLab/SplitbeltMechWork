...
[varOut.dFastStep, varOut.dSlowStep, varOut.vStride, varOut.pStride, varOut.net_imp_L, varOut.net_imp_R, varOut.net_imp_tot] =...
...
% pStride: power generated due to each leg on the CoM, the belts,and total
%   power generated by each leg
function [dFastStep, dSlowStep, vStride, pStride, net_imp_L, net_imp_R, net_imp_tot]...
...
for i=1:length(ftot)
    ...
    % Extract FA component to calculate braking and propulsion
    temp_l = fl{i,1}(:,2);
    prop_imp_L{i,1}(:,1) = trapz(temp_l(temp_l>0)/bodyWeight)/Fs;
    brake_imp_L{i,1}(:,1) = trapz(temp_l(temp_l<0)/bodyWeight)/Fs;
    net_imp_L{i,1}(:,1) = prop_imp_L{i,1}(:,1)+brake_imp_L{i,1}(:,1);
    
    temp_r = fr{i,1}(:,2);
    prop_imp_R{i,1}(:,1) = trapz(temp_r(temp_r>0)/bodyWeight)/Fs;
    brake_imp_R{i,1}(:,1) = trapz(temp_r(temp_r<0)/bodyWeight)/Fs;
    net_imp_R{i,1}(:,1) = prop_imp_R{i,1}(:,1)+brake_imp_R{i,1}(:,1);
    
    net_imp_tot{i,1}(:,1) = net_imp_L{i,1}(:,1)+net_imp_R{i,1}(:,1);
